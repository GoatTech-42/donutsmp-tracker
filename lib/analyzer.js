...
const TRAIN_WORKER_PATH = path.join(__dirname, '..', 'worker', 'trainPredictorWorker.js')

class AuctionAnalyzer {
  constructor() {
    this.snapshots = []
    this.priceHistory = new Map()
    // 2 days of 5-min snapshots (288/day). Live DB has ~1381 items × 2304 rows each,
    // so any higher cap risks OOM under --max-old-space-size=512. 576 gives the
    // predictor 12× the old 96-depth window for horizon targets without leaking RAM.
    this.maxSnapshots = Number(process.env.MAX_HISTORY_SNAPSHOTS || 576)
  ...
  storePartialAuctions(auctions) {
    ...
    for (const [name, s] of Object.entries(compact)) {
      ...
      // Partial pass has no transactions yet — sales/volume stay 0 until addSnapshot.
      h.push({
        timestamp: ts,
        floor: s.floor,
        median: s.median,
  ...
  addSnapshot(auctions, transactions = []) {
    ...
    const compact = this.snapshotFilteredStats(auctions)
    const salesByItem = this._salesByItem(transactions)
    for (const [name, s] of Object.entries(compact)) {
      const history = this.priceHistory.get(name) || []
      const sold = salesByItem.get(name) || { sales: 0, volume: 0, salesValue: 0 }
      // History stores this snapshot's OWN filtered stats + completed-sales stats,
      // so feature extraction reads per-point context and never leaks live data.
      history.push({
        timestamp,
        floor: s.floor,
        median: s.median,
  ...
  // Per-item completed-sales stats for a snapshot. Used to attach historical
  // sales/volume/salesValue to each history entry so feature extraction can read
  // a point's OWN context instead of leaking the current live listings.
  _salesByItem(transactions) {
    const out = new Map()
    for (const t of transactions || []) {
      const name = t.itemName || t.item?.display_name || t.itemId || t.item?.id
      if (!name || name === 'undefined') continue
      const s = out.get(name) || { sales: 0, volume: 0, salesValue: 0 }
      s.sales += 1
      s.volume += t.count || 1
      s.salesValue += t.price || 0
      out.set(name, s)
    }
    return out
  }
  ...
  _training = false
  // Only accept a worker result that is complete, error-free, and numerically
  // sane. A crashed or NaN-poisoned training run used to overwrite the live
  // model with garbage (or wedge _training forever on a hang).
  _isValidWorkerResult(result) {
    if (!result || typeof result !== 'object') return false
    if (!result.save || typeof result.save !== 'object') return false
    if (result.train && result.train.error) return false
    const sampleW = result.save.pricePredictor?.weights?.[0]?.[0]?.[0]
    if (sampleW !== undefined && !Number.isFinite(sampleW)) return false
    const sampleB = result.save.pricePredictor?.biases?.[0]?.[0]
    if (sampleB !== undefined && !Number.isFinite(sampleB)) return false
    return true
  }
  ...
  trainPredictor(auctions) {
    ...
    this._training = true
    // Offload the CPU-bound epoch loop to a worker thread so it never blocks the
    // HTTP/socket event loop. The worker trains a snapshot of the model and
    // returns an updated save() which we re-load on the main thread.
    return this._trainInWorker(auctions).finally(() => {
      this._training = false
    })
  }

  _trainInWorker(auctions) {
    return new Promise((resolve, reject) => {
      let settled = false
      const worker = new Worker(TRAIN_WORKER_PATH, {
        workerData: {
          modelState: this.predictor.save(),
          priceHistories: Object.fromEntries(this.priceHistory),
          auctions
        }
      })
      const finish = fn => {
      ...
      worker.once('message', result => {
        finish(() => {
          if (this._isValidWorkerResult(result)) {
            this.predictor.load(result.save)
            resolve(result.train ? result.train : result)
          } else {
            console.warn('[Analyzer] Discarding invalid train worker result', result && result.train && result.train.error ? result.train.error : '(malformed/NaN)')
            resolve({ skipped: true, reason: 'invalid_worker_result' })
          }
        })
      })
      // Belt & braces: a wedged worker must not stall training forever.
      const killer = setTimeout(() => {
        finish(() => {
          console.warn('[Analyzer] Train worker timed out; terminating')
          resolve({ skipped: true, reason: 'worker_timeout' })
        })
      }, 180000)
      killer.unref?.()
      ...
      worker.once('exit', code => {
        if (!settled) {
          finish(() => reject(new Error(`Train worker exited without result (code ${code})`)))
        }
      })
    })
  }
  ...
  buildMarket(auctions, transactions = []) {
    ...
    for (const name of names) {
      ...
      // Troll-sale guard. Sales are the trust anchor, but trolls transact at joke
      // prices too, and at small n (5-9 sales) IQR is inactive (<10 samples) and a
      // couple of joke sales can drag the median. MAD handles spread-out samples,
      // but the degenerate case (five identical $100 sales + two $10M jokes) falls
      // through every robust filter - so also drop sales sitting >5x either side
      // of the filtered ask median when enough asks exist to cross-check.
      let salesTrusted = madFilter(salePrices)
      if (asks.length >= 3) {
        const askMed = percentile([...asks].sort((a, b) => a - b), 0.5)
        if (askMed > 0) salesTrusted = salesTrusted.filter(v => v >= askMed / 5 && v <= askMed * 5)
      }

      // Transaction-anchored fairValue: need 5+ trusted sales, otherwise trimmed ask median.
      let fairValue, fairValueSource
      if (salesTrusted.length >= 5) {
        fairValue = percentile(
          [...salesTrusted].sort((a, b) => a - b),
          0.5
        )
      ...
      // Volatility from filtered set (trusted sales if enough, otherwise filtered asks).
      const deviations = salesTrusted.length >= 5 ? salesTrusted : asks
      ...
      let confidence = Math.min(
        100,
        salesTrusted.length * 12 + asks.length * 1.5 + Math.min(20, history.length)
      )
  ...
  // Investability score 0-100: what to invest in right now.
  // Combines (a) flip ROI if this item is a current snipe/craft, (b) predicted
  // 24h direction + calibrated confidence, (c) liquidity (listings/sales depth),
  // (d) volatility risk. Honest, transparent weights — not a black box.
  computeInvestability(item) {
    const byName = this.lastFlipDetection?.flips || []
    const flipMap = new Map(byName.map(f => [f.name, f]))
    const flip = flipMap.get(item.name)
    const flipScore = flip ? Math.min(100, flip.roi * 2 + flip.profit / 5000 * 10) : 0
    const pred = item.prediction
    let predScore = 50
    if (pred && pred.trend) {
      const conf = pred.confidence || 50
      if (pred.trend === 'UP') predScore = 50 + conf * 0.5
      else if (pred.trend === 'DOWN') predScore = 50 - conf * 0.5
      else predScore = 50 + (conf - 50) * 0.2
      predScore = Math.max(0, Math.min(100, predScore))
    } else {
      // No prediction yet: neutral, slightly penalize low confidence
      predScore = Math.max(20, Math.min(60, (item.confidence || 0) * 0.6))
    }
    const liquidity = Math.min(100, (item.listings || 0) * 2 + (item.sales || 0) * 8)
    const risk = this.riskFor(item)
    const riskScore = 100 - risk.score
    const score = Math.round(flipScore * 0.4 + predScore * 0.3 + liquidity * 0.2 + riskScore * 0.1)
    return { score: Math.max(0, Math.min(100, score)), flip: flip || null, prediction: pred || null, liquidity: Math.round(liquidity), risk }
  }
