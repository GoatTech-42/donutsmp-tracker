...
class AuctionAnalyzer {
  ...
  detectFlips(auctions, transactions = []) {
    ...
    for (const recipe of RECIPES) {
      ...
      // Craft sell must be grounded: a single troll listing (e.g. Mud Brick
      // Stairs @ 804M) with 0 transactions makes every craft look infinite.
      // Mid-scan transactions==0 → craftResult is unanchored; require a tight
      // sanity cap so troll listings don't pollute the board.
      const craftHasSignal = result.sales >= 1 || result.filteredListings >= 3;
      if (!craftHasSignal) continue;
      if (result.sales < 3 && result.confidence < 30 && result.filteredListings < 5) continue;
      // Hard troll cap: if the craft's sell unit implies >50x ingredient cost,
      // it's a meme listing, not a real price. No one buys Mud Brick Stairs
      // for 804M when mud+sticks cost 12K. (Skipped entirely while transactions=0
      // gives transactions a chance to anchor fairValue properly.)
      const estSellUnit = result.fairValue || result.floor;
      if (transactions.length === 0 && result.sales === 0 && estSellUnit > Math.max(cost * 50, 500000)) continue;
      let cost = 0;
      const ingredients = [];
