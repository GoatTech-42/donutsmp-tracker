...
function buildMarketContext(intel) {
  ...
  if (craftFlips.length) {
    ...
    for (const f of craftFlips) {
      ...
      lines.push(
        `${f.name} | ${ingredients} → ${compact(f.totalCost)} | SELL: ${compact(f.sellPrice)} | +${compact(f.profit)} | ROI:${f.roi}% | ${f.risk.label}`
      )
    }
  }
  ...
  if (marketFlips.length) {
    ...
    for (const f of marketFlips) {
      lines.push(
        `${f.name} | Floor:${compact(f.buyPrice)} Fair:${compact(f.sellPrice)} | ${f.listings} list/${f.volume || f.sales || 0} sold | Vol:${f.volatility || 0}% | ${f.risk.label}`
      )
    }
  }
  ...
  if (intel.enchantPremiums?.length) {
    ...
    for (const e of intel.enchantPremiums.slice(0, 6)) {
      lines.push(
        `${e.enchantName} ${e.level}: ×${e.valueMultiplier?.toFixed(2)} (+${compact(e.premiumOverBase)}), ${e.totalSales} sales`
      )
    }
  }
  ...
  if (intel.predictions?.length) {
    ...
    for (const p of sorted.slice(0, 6)) {
      lines.push(
        `${p.name}: ${compact(p.current)}→${compact(p.predicted)} Δ${p.change}% ${p.trend} (${p.confidence}%)`
      )
    }
  }
  ...
  if (intel.outliers?.length) {
    ...
    for (const o of intel.outliers.slice(0, 6)) {
      lines.push(
        `${o.name}: Z:${o.zScore} Dev:${o.deviation}% ${o.iqrOutlier ? 'IQR' : ''} ${o.direction} (${o.sales} sales)`
      )
    }
  }
  ...
  if (intel.anomalies?.length) {
    ...
    for (const a of intel.anomalies.slice(0, 4)) {
      lines.push(
        `${a.item}: ${a.type} score:${a.anomalyScore}% dev:${a.deviation}% ${compact(a.currentPrice)} vs avg ${compact(a.avgPrice)}`
      )
    }
  }
