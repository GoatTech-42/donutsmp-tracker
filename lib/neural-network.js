...
class MarketPredictor {
  ...
  predict(itemName, priceHistory, allAuctions = []) {
    ...
    return { currentPrice, predictedPrice: Math.max(1, predictedPrice), expectedChange: Math.round(expectedChange), expectedChangePct: Math.round(expectedChangePct * 100) / 100, trend: predictedTrend, confidence: Math.round(cal), timeHorizon: horizonLabel }
  }
