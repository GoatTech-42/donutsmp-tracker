class NeuralNetwork {
  ...
  batchNormForward(x, layerIdx, training) {
    ...
    return x.map((v, i) => this.bnGamma[layerIdx][i] * (v - mean) / Math.sqrt(variance + 1e-5) + this.bnBeta[layerIdx][i]);
  }
...
class MarketPredictor {
  ...
  train(priceHistories, auctions) {
    ...
    if (recent.length > 20) {
      const replaySample = this.experienceReplay.length > 0
  ...
  detectStatisticalOutliers(priceHistories, auctions) {
    ...
    for (const [itemName, history] of Object.entries(priceHistories)) {
      ...
      const prices = history.slice(-20).map(h => h.price || h.floor).filter(p => p > 0);
