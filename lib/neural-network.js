class NeuralNetwork {
  ...
  batchNormForward(x, layerIdx, training) {
    ...
    if (training) {
      ...
      variance = x.reduce((sum, v) => sum + (v - mean) ** 2, 0) / x.length
      this.bnMean[layerIdx] = 0.9 * this.bnMean[layerIdx] + 0.1 * mean
      this.bnVar[layerIdx] = 0.9 * this.bnVar[layerIdx] + 0.1 * variance
    } else {
      mean = this.bnMean[layerIdx]
      variance = this.bnVar[layerIdx]
    }
    return x.map(
      (v, i) =>
        (this.bnGamma[layerIdx][i] * (v - mean)) / Math.sqrt(variance + 1e-5) + this.bnBeta[layerIdx][i]
    )
  }
  ...
  backward(target) {
    ...
    let errors = this.outputSize === 1 ? [target[0] - output[0]] : output.map((o, i) => target[i] - o)
...
class MarketPredictor {
  ...
  extractFeatures(itemName, priceHistory, currentAuctions, allAuctions = []) {
    ...
    const momentum1 =
      prices.length >= 2
        ? (prices[prices.length - 1] - prices[prices.length - 2]) / (prices[prices.length - 2] || 1)
        : 0
    const momentum5 =
      prices.length >= 6
        ? (prices[prices.length - 1] - prices[prices.length - 6]) / (prices[prices.length - 6] || 1)
        : 0
    ...
    const avgListingPrice =
      listings > 0
        ? itemAuctions.reduce((s, a) => s + (a.pricePerUnit || a.price / a.count), 0) / listings
        : 0
    ...
    const listingsChange =
      priceHistory.length >= 2 ? (listings - (priceHistory[priceHistory.length - 2].listings || 0)) / 10 : 0
    const salesChange =
      priceHistory.length >= 2
        ? ((priceHistory[priceHistory.length - 1].sales || 0) -
            (priceHistory[priceHistory.length - 2].sales || 0)) /
          10
        : 0

    const priceAcceleration =
      prices.length >= 3
        ? (prices[prices.length - 1] - 2 * prices[prices.length - 2] + prices[prices.length - 3]) /
          (prices[prices.length - 3] || 1)
        : 0
    const volumeAcceleration =
      volumes.length >= 3
        ? (volumes[volumes.length - 1] - 2 * volumes[volumes.length - 2] + volumes[volumes.length - 3]) /
          (volumes[volumes.length - 3] || 1)
        : 0
  ...
  prepareTrainingData(priceHistories, auctions) {
    ...
    for (const [itemName, history] of Object.entries(priceHistories)) {
      ...
      for (let i = 15; i < history.length - 1; i++) {
        ...
        const trend =
          (future.price || future.floor) > (past[past.length - 1].price || past[past.length - 1].floor)
            ? [1, 0, 0]
            : (future.price || future.floor) < (past[past.length - 1].price || past[past.length - 1].floor)
              ? [0, 0, 1]
              : [0, 1, 0]
  ...
  train(priceHistories, auctions) {
    ...
    // Experience replay: mix new data with replayed old data for stability
    if (recent.length > 20) {
      const replaySample =
        this.experienceReplay.length > 0
          ? this.experienceReplay.slice(-Math.min(100, Math.floor(recent.length * 0.3)))
          : []
      const mixed = [...recent, ...replaySample]
      // Push latest to replay buffer
      this.experienceReplay.push(...recent.slice(-50))
      if (this.experienceReplay.length > this.maxExperienceBuffer) {
        this.experienceReplay = this.experienceReplay.slice(-this.maxExperienceBuffer)
      }
      ...
      const priceLoss = this.pricePredictor.train(priceInputs, priceTargets, 60, 64)
      const trendLoss = this.trendPredictor.train(priceInputs, trendTargets, 40, 64)

      // Anomaly detector trained on price deviation samples
      const anomalyInputs = mixed.map(p => p.features.slice(0, 24))
      const anomalyTargets = mixed.map(p => {
        const deviation = Math.abs(p.features[0] - p.features[1]) / (p.features[1] || 0.001)
        return [Math.min(1, deviation * 10)]
      })
      if (anomalyTargets.some(t => t[0] > 0.1)) {
        this.anomalyDetector.train(anomalyInputs, anomalyTargets, 15, 64)
      }
      ...
      return {
        ...
        replaySamples: replaySample.length,
        epochs: this.pricePredictor.epochs
      }
    }

    // Fallback: train only on recent data
    ...
    const anomalyInputs = recent.map(p => p.features.slice(0, 24))
    const anomalyTargets = recent.map(p => {
      const deviation = Math.abs(p.features[0] - p.features[1]) / (p.features[1] || 0.001)
      return [Math.min(1, deviation * 10)]
    })
    if (anomalyTargets.some(t => t[0] > 0.1)) {
  ...
  predict(itemName, priceHistory, currentAuctions, allAuctions = []) {
    ...
    const currentPrice =
      priceHistory[priceHistory.length - 1]?.price || priceHistory[priceHistory.length - 1]?.floor || 0
  ...
  detectStatisticalOutliers(priceHistories, auctions) {
    ...
    for (const [itemName, history] of Object.entries(priceHistories)) {
      ...
      if (Math.abs(zScore) > 2 || iqrOutlier) {
        const avgPrice = history.slice(-10).reduce((s, h) => s + (h.price || h.floor), 0) / 10
        const saleCount = currentAuctions.length
        const salesValue = currentAuctions.reduce((s, a) => s + a.price, 0)
        ...
        outliers.push({
          ...
          iqrOutlier,
          iqrLower: Math.round(iqrLower),
          iqrUpper: Math.round(iqrUpper),
          sales: saleCount,
          salesValue,
          deviation: Math.round(((currentPrice - avgPrice) / (avgPrice || 1)) * 100 * 100) / 100,
          direction: currentPrice > mean ? 'overpriced' : 'underpriced'
        })
      }
    }
