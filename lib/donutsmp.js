...
async function fetchAllAuctions(maxPages = Number(process.env.AUCTION_PAGES || 9999), onProgress) {
  if (!API_KEY) throw new UpstreamError('DONUTSMP_API_KEY is not configured', 503);
  const rows = [];
  let page = 1;
  ...
  while (page <= maxPages) {
    ...
    for (let p = page; p <= batchEnd; p++) {
      batchPromises.push(auctionList(p).then(data => ({ page: p, rows: (data.result || []).map(r => normalizeAuction(r)) })).catch(e => ({ page: p, rows: [], error: e.message })));
    }
