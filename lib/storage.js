...
const getMarketStatsHistory = db.prepare(`SELECT * FROM market_stats ORDER BY timestamp DESC LIMIT ?`);
const getFlipHistory = db.prepare(`SELECT * FROM flip_history ORDER BY timestamp DESC LIMIT ?`);
const getFlipHistoryByItem = db.prepare(`SELECT * FROM flip_history WHERE item_name = ? ORDER BY timestamp DESC LIMIT ?`);
...
function loadAllPriceHistories() {
  const itemNames = db.prepare(`SELECT DISTINCT item_name FROM snapshots`).all().map(r => r.item_name);
  const histories = {};
...
function getFlipHistory(limit = 100) {
  return getFlipHistory.all(limit);
}

function getFlipHistoryByItem(itemName, limit = 50) {
  return getFlipHistoryByItem.all(itemName, limit);
}

function getMarketStatsHistory(limit = 100) {
  return getMarketStatsHistory.all(limit);
}
