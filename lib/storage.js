...
const getFlipHistoryByItemStmt = db.prepare(`SELECT * FROM flip_history WHERE item_name = ? ORDER BY timestamp DESC LIMIT ?`);
...
function saveEnchantmentStats(snapshot) {
  ...
  for (const tx of snapshot.transactions) {
    ...
    if (!comboStats.has(comboKey)) {
      comboStats.set(comboKey, { item: tx.itemName, enchants: enchantEntries, prices: [], baseEstimate: basePrice });
    }
